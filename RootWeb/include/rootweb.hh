...
class RootWItem
{
  public:
	...
	virtual ostream& dump( ostream& output ) {
		return output;
	};
	bool taken;
};
...
class RootWFileList : public RootWItem
{
  ...
  public:
	RootWFileList() {};
	template<class I> RootWFileList( I begin, I end ) {
		addFileNames( begin, end );
	}
	template<class I> RootWFileList( I begin, I end, const string& description ) : description_( description ) {
		addFileNames( begin, end );
	}
	void addFileName( string newFileName ) {
		fileNames_.push_back( newFileName );
	}
	...
	const std::list<string>& getFileNames() const {
		return fileNames_;
	}
	string getDescription() {
		return description_;
...
class RootWTextFile : public RootWFile
{
  ...
  public:
	...
	void addText( stringstream& newText ) {
		myText_ << newText.str() ;
	};
	ostream& dump( ostream& output );
};
...
class RootWBinaryFile : public RootWFile
{
  ...
  public:
	...
	void setOriginalFile( string newFile ) {
		originalFileName_ = newFile ;
	};
	ostream& dump( ostream& output );
};
